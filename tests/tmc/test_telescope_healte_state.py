...
scenarios("../features/tmc/check_telescope_healthstate.feature")
...
def telescope_on(central_node_low, event_recorder):
    ...
    state["central_node"] = central_node_low.central_node
    central_node_low.move_to_on()
    event_recorder.subscribe_event(state["central_node"], "telescopeState")
    assert event_recorder.has_change_event_occurred(
        state["central_node"], "telescopeState", DevState.ON
    )
...
def set_master_health(
    simulator_factory, csp_state=None, sdp_state=None, mccs_state=None
):
    """Set master devices healthstate"""
    csp, sdp, mccs = get_master_device_simulators(simulator_factory)
    state["csp"], state["sdp"], state["mccs"] = csp, sdp, mccs
    state["csp_state"], state["sdp_state"], state["mccs_state"] = (
        csp_state,
        sdp_state,
        mccs_state,
    )
...
def check_telescope_health_state(event_recorder, expected_state):
    """Verify the telescope healthstate"""
    event_recorder.subscribe_event(
        state["central_node"], "telescopeHealthState"
    )
    assert event_recorder.has_change_event_occurred(
        state["central_node"],
        "telescopeHealthState",
