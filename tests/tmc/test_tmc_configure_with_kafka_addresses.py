"""
Module: test_tmc_configure_with_kafka_address
...
system. The scenario includes steps to set up the TMC, prepare a subarray in
the IDLE observation state, set the kafka addresses in SDP receiveAddresses
attribute and configure it for a scan. The completion of the configuration is
verified  by checking that the subarray transitions to the READY observation
state.
"""
import json
...
def given_set_receive_addresses_on_sdp(
    central_node_low: CentralNodeWrapperLow,
):
    """Set up a subarray in the IDLE obsState."""
    set_receive_address(central_node_low)
    ...
    receive_address = get_centralnode_input_json(
        "ReceiveAddresses_with_kafka_low"
    )
